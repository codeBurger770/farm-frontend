...
import { deleteEventOrRiskEventThunk } from '../../redux/currentProject/thunks'

export default function EventsAndRiskEventsPage() {
    const { projectId } = useParams()

    const dispatch = useDispatch()
    const { eventsAndRiskEvents } = useSelector(state => state.currentProject)
    ...
    return (
        <>
            ...
            <div className="container-fluid py-3">
                <h1 className="mb-3">События и риск-события</h1>
                {!eventsAndRiskEvents.length ? (
                    <div>Событий и риск-событий пока нет...</div>
                        ...
                        <div className="table-responsive">
                            <table className="table">
                                ...
                                <tbody>
                                    {eventsAndRiskEvents.map(eventOrRiskEvent => (
                                        <tr key={eventOrRiskEvent.id}>
                                            ...
                                            <td>
                                                <div className="d-flex">
                                                    <Link className="btn btn-primary mr-3" to={`/projects/${projectId}/events-and-risk-events/${eventOrRiskEvent.id}/edit`}>
                                                    ...
                                                    <button className="btn btn-danger" onClick={e => dispatch(deleteEventOrRiskEventThunk(projectId, eventOrRiskEvent.id))}>
