...
import { updateActivityThunk } from '../../redux/currentProject/thunks'

export default function UpdateActivityPage() {
    const { projectId, activityId } = useParams()

    const dispatch = useDispatch()
    const { forms, eventsAndRiskEvents, logicalOperators, activities } = useSelector(state => ({
        forms: state.forms,
    ...
    useEffect(() => {
        ...
        dispatch(setTypeAction(activity.type))
        dispatch(setEventOrRiskEventOrLogicalOperatorIdAction(
            activity.type === 'DELETE_LOGICAL_OPERATOR'
                ? activity.logicalOperatorId
                : activity.eventOrRiskEventId
        ))
        dispatch(setDescriptionAction(activity.description))
        dispatch(setCostAction(activity.cost))
        dispatch(setProbabilityAction(activity.probability))
        dispatch(setConsequencesAction(activity.consequences))
    ...
    const handleSubmit = e => {
        e.preventDefault()
        dispatch(updateActivityThunk(projectId, activityId))
    }
    ...
    return (
        <>
            ...
            <div className="container-fluid py-3">
                <h1 className="mb-3">Изменить мероприятие</h1>
